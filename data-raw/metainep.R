...
scrapeass <- \(assunto) {
  burl <- paste0(assuntos[assunto,]$url,"/resultados")
  print(burl)
  checasubres <- httr::GET(burl)
  if(checasubres$status_code==404){
    burl <- gsub("/resultados","",burl)
  }
  ...
  data.frame(
    assunto_id=assunto,
    periodo=gsub(".*(\\d{4})[^\\d].*","\\1",tabelaurl),
    taburl=tabelaurl,
    tabela=tabelalabels
  )
  } else {
  numabas <- abasres[!is.na(as.numeric(abasres))]

  zipsassunto <- \(naba) {

    localizador <- paste0('div.tab.swiper-slide-next ',paste0(rep('+ div.tab',naba-1),collapse= " "),' > a')
  ...
  #Clica em todas as abas para carregar os links
    try(lapply(1:length(numabas),zipsassunto))
